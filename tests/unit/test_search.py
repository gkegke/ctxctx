...
def search_setup_fs(fs):
    ...
    """
    # Get a fresh, mutable default config for this test run
    test_config = get_default_config()

    root_path = Path("/project")  # Changed to Path object
    fs.create_dir(root_path)
    fs.create_file(root_path / "main.py", contents="def foo(): pass")
    fs.create_file(root_path / "README.md")
    fs.create_file(root_path / "LICENSE")

    src_dir = root_path / "src"
    fs.create_dir(src_dir)
    fs.create_file(src_dir / "utils.py", contents="class MyClass: pass")
    fs.create_file(src_dir / "temp_data.log")  # Will be ignored by substring
    fs.create_file(src_dir / "README.md")
    fs.create_file(src_dir / ".gitignore")  # Added for new test

    docs_dir = root_path / "docs"
    fs.create_dir(docs_dir)
    fs.create_file(docs_dir / "guide.md")
    fs.create_file(docs_dir / "LICENSE")

    node_modules_dir = root_path / "node_modules"
    fs.create_dir(node_modules_dir)  # Ignored by default
    fs.create_file(node_modules_dir / "lib.js")

    git_ignored_dir = root_path / "git_ignored_dir"
    fs.create_dir(git_ignored_dir)
    fs.create_file(git_ignored_dir / "ignored_file.txt")

    fs.create_file(
        root_path / ".gitignore",  # Use Path object
        contents="git_ignored_dir/\n*.log\n.gitignore\n",
    )  # Simulate gitignore
    fs.create_file(root_path / "app.log")  # A top-level log file

    # Changed: Set ROOT and other config on the *test_config* object
    test_config.root = root_path
    test_config.substring_ignore_patterns.append("temp_data")  # Ensure this is ignored
    test_config.search_max_depth = 5  # Default, sufficient for tests

    # Changed: create_is_ignored now takes a config_obj argument and passes it to IgnoreManager
    def create_is_ignored(
        config_obj: Config, force_include_patterns: List[str] = None
    ) -> Callable[[Path], bool]:  # Changed Path type hint
        """
        Helper to create an is_ignored callable for tests with specific
        force_include_patterns.
        """
        # Pass the Config object, and the force_include_patterns
        manager = IgnoreManager(config_obj, force_include_patterns=force_include_patterns or [])
        return manager.is_ignored

    return root_path, create_is_ignored, test_config
...
def test_find_matches_glob_pattern(search_setup_fs):
    ...
    assert any(m["path"] == root / "README.md" for m in matches)
    assert any(m["path"] == root / "docs" / "guide.md" for m in matches)
...
def test_find_matches_force_include_glob(search_setup_fs):
    ...
    assert any(m["path"] == root / "app.log" for m in matches)
    for m in matches:
        assert m["line_ranges"] == []
...
def test_find_matches_force_include_simple_filename_finds_all_matches(search_setup_fs):
    """
...
def test_find_matches_force_include_simple_filename_with_line_ranges_finds_all_matches(
    search_setup_fs,
):
    """
    ...
    # because the early exit for simple filenames is removed, and is_ignored does not ignore them.
    assert len(matches) == 2
    assert any(m["path"] == root / "LICENSE" and m["line_ranges"] == [(1, 5)] for m in matches)
    assert any(
        m["path"] == root / "docs" / "LICENSE" and m["line_ranges"] == [(1, 5)] for m in matches
    )


def test_find_matches_force_include_simple_filename_not_at_root_finds_in_subdir(
    search_setup_fs, fs
):
    """
    ...
    non_root_file = sub_dir / "non_root_file.txt"
    fs.create_file(non_root_file)
...
def test_find_matches_force_include_dotted_filename_finds_all_matches(search_setup_fs):
    """
...
def test_find_matches_force_include_path_with_slash_still_searches_deeply(search_setup_fs):
    ...
    ignored_deep_file = root / "git_ignored_dir" / "ignored_file.txt"

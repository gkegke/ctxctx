...
def setup_tree_fs(fs):
    ...
    fs.create_file(
        os.path.join(root_path, "nested_ignored_dir", "secret.txt")
    )  # This file is NOT ignored by default config
...
def test_generate_tree_string_basic(setup_tree_fs):
    ...
    # assert tree.index("node_modules") < tree.index("nested_ignored_dir")
...
def test_generate_tree_string_max_depth(setup_tree_fs):
    ...
    # With current logic: max_depth=0 -> current_depth=0 is okay, current_depth=1 will
    # be > max_depth=0.
    # So `src` would be at depth 1 and skipped. This means only root is shown if root has
    # no direct files.
    # The common expectation of `max_depth=0` for a tree is "don't recurse at all,
    # just list top-level files/dirs".
